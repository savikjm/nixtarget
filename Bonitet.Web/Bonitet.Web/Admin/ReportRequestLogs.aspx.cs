...
{
    public partial class ReportRequestLogs : System.Web.UI.Page
    {
        ...
        {
            var total = DALHelper.GetTotalReportRequestLogs(filter);
            ...
            var report_requests = DALHelper.GetRequestLogsByPage(skip, pageSize, filter);
        ...
        {
            if (filter > 1)
                Response.Redirect("ReportRequestLogs.aspx?page=" + page.ToString() + "&filter=" + filter, true);
            else
                Response.Redirect("ReportRequestLogs.aspx?page=" + page.ToString(), true);
        }
        ...
        protected void search_selector_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Response.Redirect("ReportRequestLogs.aspx?filter=" + selected);
        }
