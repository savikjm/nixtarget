    ...
    public partial class PrepayPackEdit : System.Web.UI.Page
    {
        ...
        public void PopulateData(int upackid, int uid)
        {
            ...
            if (prepay_pack != null)
            {
                ...
                post_paid.Checked = prepay_pack.IsPostPaid;
            }
            else
            {
                Response.Redirect("/Admin/Default.aspx");
        ...
        protected void submit_buttn_Click(object sender, EventArgs e)
        {
            ...
            if (!String.IsNullOrEmpty(id) && !String.IsNullOrEmpty(packid))
            {
                ...
                if (error == false)
                {
                    if (DALHelper.UpdatePrepayPack(upackid, uid, Convert.ToDateTime(DateStart), Convert.ToDateTime(DateEnd), Convert.ToInt32(Pack), Comment, IsPostPaid))
