...
function GeneratePDF(type, force) {
    var companyid = $('.cur_company_id').val();
    var embs = $('.cur_embs').val();
    var client_embs = $('.client_embs').val();
    var forced_year = $('.forced_year').val();

    if (type == 1) {
        $('.loader_crm_pdf').show();
        $('.crm_pdf_waiting').show();

        $('.crm_pdf_confirm').hide();
        $('#generate_crm_pdf').hide();
        $('.crm_pdf_error').hide();
        $('#crm_pdf_error_message').hide();
    } else if (type == 2) {
        ...
        $('#blokada_success_bontitet').show();
        $('#blokada_info_bonitet').hide();
    }
    ...
    $.ajax({
        ...
        data: "create_pdf=1&type=" + type + "&companyid=" + companyid + "&embs=" + embs + "&userTicket=" + userTicket + "&clientEmbs=" + client_embs + "&force_crm=" + force + "&forced_year=" + forced_year,
        dataType: 'text',
        success: function (res) {
            if (res.indexOf("error") > -1) {
                if (type == 1) {
                    $('.crm_pdf_error').text(res);
                    if (res.indexOf("Licence error") > -1) {
                        $('.crm_pdf_error').text("Грешка!");
                        $('#crm_pdf_error_message').show();
                    }
                    $('.crm_pdf_error').show();

                    $('.loader_crm_pdf').hide();
                    $('.crm_pdf_waiting').hide();
                    $('.crm_pdf_confirm').hide();
                    $('#generate_crm_pdf').hide();
                } else if (type == 2) {
...
function ResetFieldsPDF() {
    ...
    $('.loader_short_pdf').hide();
    $('#short_pdf_error_message').hide();
    $('#short_pdf_info_bonitet').show();
}
function ResetFieldsCRMPDF() {
    $('.crm_pdf_error').text("Грешка!");
    $('.crm_pdf_confirm').show();
    $('#generate_crm_pdf').show();

    $('.crm_pdf_waiting').hide();
    $('.crm_pdf_error').hide();
    $('.loader_crm_pdf').hide();
    $('#crm_pdf_error_message').hide();
    $('#crm_pdf_error_message1').hide();
}
...
var request_userid;
var request_id;
function FillDataForRequestMail(embs, userid, id) {
    request_embs = embs;
...
function SendRequestMail() {
    $.ajax({
        type: 'POST',
        url: "/DocumentHelper.ashx",
        data: "send_request_mail=1&embs=" + request_embs + "&userid=" + request_userid + "&request_id=" + request_id,
        dataType: 'text',
        success: function (res) {
            if (res == "Mail Sent")
                alert("Мејлот е испратен.");
            else
                alert("Грешка!");
        },
        error: function (a, b, c) {
...
function SendRequestMailNoData() {
    $.ajax({
        type: 'POST',
        url: "/DocumentHelper.ashx",
        data: "send_request_mail=1&no_data=1&embs=" + request_embs + "&userid=" + request_userid + "&request_id=" + request_id,
        dataType: 'text',
        success: function (res) {
            if (res == "Mail Sent"){
                if (window.confirm("Мејлот е испратен, стиснете ОК за да се ажурираат податоците."))
                    window.location = window.location;
            }
            else
                alert("Грешка!");
...
function SendChargeMail() {
    $.ajax({
        type: 'POST',
        url: "/DocumentHelper.ashx",
        data: "send_charge_mail=1&embs=" + request_embs + "&userid=" + request_userid + "&request_id=" + request_id,
        dataType: 'text',
        success: function (res) {
            if (res == "Mail Sent") {
                if (window.confirm("Мејлот е испратен."))
                    window.location = window.location;
            }
            else
                alert("Грешка!");
    ...
    function DeleteFromList() {
        $.ajax({
            type: 'POST',
            url: "/DocumentHelper.ashx",
            data: "delete_from_list=1&embs=" + request_embs + "&request_id=" + request_id,
            dataType: 'text',
            success: function (res) {
                if (res == "Deleted") {
                    if (window.confirm("Барањето е избришано, стиснете ОК за да се ажурираат податоците."))
                        window.location = window.location;
                }
            },
            error: function (a, b, c) {
    ...
    $('document').ready(function () {
        ...
        if ($('#start_date_container1').length > 0 && $('#end_date_container1').length > 0) {
            var dateString = $('.sDate').val().split('.');
            ...
            $('#start_date_container1').datetimepicker({
                format: "YYYY-MM-DD",
                defaultDate: sDate
            });
            $('#end_date_container1').datetimepicker({
                format: "YYYY-MM-DD",
                defaultDate: eDate
            });
        }
